...
for(i in 1:length(regions)){
  river_xyz <- readRDS(paste0(data_path,'/z_',regions[i], '_xy_all_2.rds'))
  ...
  river_xyz_av[, segments_count := nrow(.SD), .(hybas_l12)]
  river_xyz_av <- river_xyz_av[segments_count > 1]
  river_xyz_av <- river_xyz_av[ord_clas == 1]
  # zmin is where min z of next_down == 0
  river_xyz_av[, zmin := min(z_av[which.min(next_down)]), .(hybas_l12)]
  # zmax is where z where max(dist_up_km)
  river_xyz_av[, zmax := max(z_av[which.max(dist_dn_km)]), .(hybas_l12)]
  # l is max(dist_up_km)
  river_xyz_av[, l_max := max(dist_up_km)-min(dist_up_km), .(hybas_l12)]
  river_xyz_av[, l := dist_up_km-min(dist_up_km), .(hybas_l12)]
  ...
  river_xyz_av[, NCI := median(v_dev), .(hybas_l12)]
  saveRDS(river_xyz_av, paste0(results_path, '/',regions[i],'_xyz_NCI_hybas_ord1.rds'))
}
