...
plot(river_pilot$geometry)


river_multi <- st_cast(river_pilot$geometry, to = 'MULTILINESTRING', ids = river_pilot$BAS_ID)

png('results/river_pilot_test.png')
plot(river_multi, col = unique(river_pilot$BAS_ID), lwd = 0.5, axes =T)
dev.off()
